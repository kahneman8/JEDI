...
import openai
from .config import (MODEL, OPENAI_API_KEY, GLOBAL_QUERY, LOCAL_QUERY,
 SEARCH_MAX_RESULTS, FETCH_TIMEOUT_SEC, MAX_WORKERS)
...
def _fetch_article(item):
 ...
 url = item.get("url")
 try:
 resp = requests.get(url, timeout=FETCH_TIMEOUT_SEC, headers={"User-Agent":
"Mozilla/5.0"})
